...
2. Analog zu 1), können wir mit der Funktion `help` die Hilfeseite für eine Funktion aufrufen (z.B. wenn wir wieder die Hilfe für die Funktion `mean()` lesen möchten, dann könnten wir auch `help(mean)` in die Konsole tippen).
3. In R Studio kann man auch auf das `Help`-Tab (Pane 4) klicken und dann einfach eine Funktion suchen (siehe Abbildung 1). 
...
Zeitformate überführt, nennt man *parsen*[^4].
...
Aufgaben deutlich schneller und effizienter erledigen. Starten Sie am besten
...
In der Forstbranche haben wir oft mit Zeitreihen zu tun. Zeitreihen sind Variablen, die in zeitlichen
Intervallen vorliegen. In der Regel monatlich, vierteljährig oder jährlich, wobei die Intervalle zwischen
den Messungen immer gleich lang sind. Wiederholungsmessungen von Forstinventuren (Forsteinrichtungen, 
...
Statistisch gesehen haben Zeitreihen Besonderheiten gegenüber anderen Variablen, da Sie in
erster Linie von Ihrer eigenen Vergangenheit abhängen (autokorreliert sind) und auch die 
Abhängigkeit anderer Variablen in der Regel nur sinnvoll ist, wenn deren Vergangenheit 
mitberücksichtigt wird (cross-correlation). Konventionelle Statistik ist oft nicht ausreichend,
um Zeitreihen zu analysieren. Angefangen mit der Datendarstellung gibt es spezifische 
Zeitreihen-Funktionen, welche auch alle in R integriert sind. Aus diesem Grund sollten Sie
Zeitreihen als solche speichern. Viele Funktionen erkennen den Datentyp und führen entsprechend
spezifische Operationen durch. Laden wir z. B. die Holzpreise für Fichte 2b (das sog. 
Leitsortiment), das Holzaufkommen dieses Sortiments und
die Preise für Nadelschnittholz vom statistischen Bundesamt^[Sie können sich die Daten auch 
selbst über die Website laden oder das Paket wiesbaden verwenden, um die Daten direkt in R 
...
Mit der Funktion `ts` werden die Daten in ein Zeitreihenobjekt überführt (geparst). Die 
Spalte mit den Jahren
ist dann nicht mehr nötig, da die Informationen zur Zeit keine eigene Variable mehr sind,
sondern zu Metainformationen werden. Die Spalten sollen nur noch Daten enthalten.
...
Das Paket `forecast` ermöglicht automatisierte Zeitreihenplots im `ggplot2` Stil. Damit ist 
auch das Problem der y-Achsenbeschriftungen gelöst. 
...
Eine Trendlinie macht hier (sowie generell in Zeitreihendaten) offensichtlich keinen Sinn.
Daher verwenden wir den sog. Loess-Glätter, um einen Trend in die Daten zu legen. Der 
Loess-Glätter ist ein sehr flexible Kurve.
