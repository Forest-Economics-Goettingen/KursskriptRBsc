...
#   - \newcommand{\sectionbreak}{\clearpage}
   ...
   - \usepackage{menukeys}
   - \pagestyle{fancy}
   - \fancyfoot[C]{Einführung in die Datenanalyse mit R (WiSe 2021/2)}
   - \fancyfoot[R]{\thepage}
output:
...
Meist verwenden wir jedoch Skripte, um den R-Code zu schreiben und ihn dann an die Konsole "zu schicken". Dies hat den Vorteil, dass alle Schritte nachvollziehbar bleiben und Analysen beliebig oft wiederholt werden können. Wenn wir R-Code in einem R-Skript geschrieben haben gibt es mehrere Möglichkeiten diesen Code auszuführen. Wir können entweder auf *Run* drücken (Abbildung \@ref(fig:rstudio-run)) oder mit der Tastenkombination *Strg + Enter* (\keys{Strg +  + \return}) den Code an die Konsole schicken.
...
# Aufgaben Wiederholen (`for`-Schleifen) {kontrollstrukturen}
